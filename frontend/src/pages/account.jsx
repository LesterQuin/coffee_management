...
export default function Staff() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      if (res.data.success) {
        // Filter to only Admins and Cashiers
        const filtered = res.data.data.filter(
          (s) => s.role.toLowerCase() === "admin" || s.role.toLowerCase() === "cashier"
        );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isCashier) return; // Cashiers cannot create users
    setLoading(true);
    try {
      await axios.post(
      ...
      <table className="min-w-full bg-white shadow rounded">
        <thead className="bg-gray-200">
          <tr>
            ...
            {!isCashier && <th className="py-2 px-4 text-center">Action</th>}
          </tr>
        </thead>
        <tbody>
          {staffList.map((s) => (
